...
var curveHash = exports.curveHash = {
...
function createPoints(_ref2) {
  ...
  data.forEach(function (d, i) {
    var dX = xScale(d[xMiddle] || d[x]);
    var dY = yScale(d[yMiddle] || d[y]);
    var pointAriaLabel = "Point at x " + d.x + " and y " + d.y;
    var renderedCustomMark = customMark && customMark({ d: d.data, xy: d, i: i, xScale: xScale, yScale: yScale });
    var markProps = customMark ? _extends(baseMarkProps, renderedCustomMark.props, {
      "aria-label": pointAriaLabel
    }) : _extends({}, baseMarkProps, {
      "key": "piece-" + i,
      "markType": "circle",
      "r": 2,
      "aria-label": pointAriaLabel
    ...
    if (renderedCustomMark && renderedCustomMark.props && !renderedCustomMark.props.markType && (!canvasRender || canvasRender(d.data, i) !== true)) {
      mappedPoints.push(_react2.default.createElement(
        "g",
        {
...
function createLines(_ref3) {
  var xScale = _ref3.xScale,
      ...
      axesData = _ref3$axesData === undefined ? [] : _ref3$axesData;
  ...
  data.forEach(function (d, i) {
    if (customMark && typeof customMark === "function") {
      ...
      if (pathString && (!customLine.interpolator || interpolator === _d3Shape.curveLinear)) {
        //FIX FOR CHROME STRAIGHT LINE BUG
        var splitPath = pathString.split("L").map(function (d) {
          return d.split(",");
        });
        if (splitPath.length > 1) {
          splitPath[0][1] = parseFloat(splitPath[0][1]).toFixed(2);
        }
        pathString = splitPath.map(function (d) {
          return d.join(",");
        }).join("L");
      }

      var markProps = _extends({}, builtInDisplayProps, baseMarkProps, {
        "markType": "path",
        "d": pathString,
...
function createAreas(_ref4) {
  ...
  data.forEach(function (d, i) {
    ...
    if (d.customMark) {
      ...
      var projectedCoordinates = d._xyfCoordinates.map(function (p) {
        return [xScale(p[0]), yScale(p[1])];
      });
      drawD = customMark({
        d: d,
    ...
    if (shouldBeValid && _react2.default.isValidElement(drawD)) {
      ...
      renderedAreas.push(_react2.default.createElement(_semioticMark.Mark, _extends({}, baseMarkProps, {
        key: renderKey,
        forceUpdate: true,
...
function clonedAppliedElement(_ref5) {
  ...
  if (!markProps.markType) {
    return _react2.default.createElement("markProps", null);
  }
